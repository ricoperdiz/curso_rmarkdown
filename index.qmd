...
O objetivo primário deste livro é servir como material de apoio para os discentes da disciplina "Produção de documentos científicos com a linguagem Rmarkdown", oferecida no Programa de Pós-graduação em Recursos Naturais (PRONAT) da Universidade Federal de Roraima (UFRR).
Além disso, outro objetivo é poder disponibilizar um material em português de qualidade, de forma a aumentar a acessibilidade e abrangência de uso desta ferramenta em nosso país, em especial na região Norte.
A maioria dos livros e tutoriais que tratam de Rmarkdown são disponibilizados na língua inglesa e, portanto, qualquer material escrito em português e de livre e fácil acesso pode ser tornar uma ferramenta relevante para o ensino de nossos estudantes.  
...
Ressalto que linguagens de marcação **não são** linguagens de programação!  
...
A primeira parte foca no ensino do Rmarkdown, enquanto a segunda focará no ensino do Quarto.
Atualmente, dedico meu esforço em produzir conteúdo para a primeira parte; a segunda parte, futuramente, poderá ser desmembrada e virar um novo livro digital.  
...
* **[Capítulo "Compilação"](04_compilacao.qmd)** ensina a compilar arquivos Rmarkdown usando o RStudio ou por meio de linhas de comando. Explica também o fluxo de trabalho por trás da compilação dos diferentes tipos de produtos.  
* **[Capítulo "Tipos de produtos"](05_output.qmd)** ensina quais são os principais tipos de produtos gerados a partir de arquivos Rmarkdown, e destrincha as principais funcionalidades de cada tipo.  
* **[Capítulo "Controle de opções em pedaços de códigos"](055_controle_codigos.qmd)** exemplifica os principais controles de código disponíveis via pacote R __knitr__. Uma série de argumentos desse pacote permitem ao usuário controlar como esse código será processado durante a compilação dos diferentes produtos.  
* **[Capítulo "Miscelânea"](056_funcoes.qmd)** é um compilado de diversas funcionalidades possíveis em arquivos Rmarkdown, ainda em fase de **(des)**organização.  
...
Utilizei o RStudio, versão 2025.05.0+496 "Mariposa Orchid", dentro do sistema operacional `r so_dad[1]`.
...
Nomes na língua inglesa serão colocados em itálico (ex., _output_).
